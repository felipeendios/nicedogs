...
class BreedsViewModel(
    ...
    private val useCaseGetBreedsByPage: UseCaseGetBreedsByPage
    ...
    override fun invokeAction(action: BreedsViewModelContract.Action) {
        when (action) {
            is BreedsViewModelContract.Action.GetBreeds
            -> getBreeds(action.lastCompletelyVisibleItemPosition)

            is BreedsViewModelContract.Action.NavigateToBreedDetails -> _breedsEvents.postValue(
                BreedsViewModelContract.Event.NavigationToBreedDetail(action.id)
            )
            ...
            is BreedsViewModelContract.Action.ChangeAlphabeticalOrder -> {
                isAZSort = !isAZSort
                _breedsEvents.postValue(
                    BreedsViewModelContract.Event.ChangeAlphabeticalOrder(
                        isAZSort
                    )
                )
            }
            ...
            if (shouldFetchNextPage(lastCompletelyVisibleItemPosition)) {
                useCaseGetBreedsByPage(page).collect { resource ->
                    ...
                    when (resource) {
                        ...
                        is Resource.Error ->
                            if (page == 0) _breedsStates.postValue(
                                BreedsViewModelContract.State.ErrorState(
                                    resource.message
                                )
                            )
                            else _breedsEvents.postValue(
                                BreedsViewModelContract.Event.PaginationError(
                                    resource.message
                                )
                            )

                        is Resource.Success ->
                            if (page == 0 && resource.data.isNullOrEmpty()) _breedsStates.postValue(
                                BreedsViewModelContract.State.EmptyState
                            )
                            else _breedsStates.postValue(
                                BreedsViewModelContract.State.SuccessState(
                                    resource.data
                                )
                            )
                    }
    ...
    private fun shouldFetchNextPage(lastCompletelyVisibleItemPosition: Int) =
        lastCompletelyVisibleItemPosition >= totalBreeds
}
